...
export default function GallerySelecetion() {
  ...
  const handleInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (images.length + e.target.files!.length <= 9) {
      ...
      const fileArray = [...target].map((value: Blob) => URL.createObjectURL(value))
  ...
  const handleSortDragOver = (index: number, e: React.DragEvent<HTMLDivElement>) => {
    e.preventDefault();
  ...
  return (
    <div className="flex flex-col gap-10">
      <label className={`${images.length == 0 ? `flex flex-col` : `grid grid-cols-3 gap-1`} w-full h-fit p-1 border-2  rounded-md cursor-pointer items-center`}
        htmlFor="dropzone"
        onDrop={handleDrop}
          ...
          images.length == 0 ?
            ...
            images.map((value, index) => {
              return (
                ...
                >
                  <div className="absolute size-4 top-1 right-1 rounded-full bg-gray-400/80" onClick={() => handleDelete(index)}>
      ...
      <input id="dropzone" ref={fileRef} type="file" onChange={handleInput} multiple accept="image/*" className="hidden cursor-none" />
      <label
        htmlFor="dropzone"
      ...
      <section className="w-full flex justify-around">
        <button onClick={() => setGrid(true)} className={`px-10 py-2 rounded-md ${grid ? 'bg-gray-200 border border-black ' : 'bg-transparent  '}`}>
          <GridIcon />
          그리드
        </button>
        <button onClick={() => setGrid(false)} className={`px-10 py-2 rounded-md ${!grid ? 'bg-gray-200 border border-black ' : 'bg-transparent  '}`}>
          <GridIcon />
          페이징</button>
      </section>
      <button onClick={handleSave} className="bg-button p-2 rounded-md text-white hover:bg-button/80">저장하기</button>
    </div >
  )
}