    ...
    <div className="flex flex-col gap-8  p-2  h-36">
      <div className="flex flex-row items-center gap-14">
        ...
        <div className="flex gap-1">
          ...
          >{font}
            <div className={`absolute flex flex-col top-full w-24  -left-[1px] rounded-b-md text-gray-800 bg-white   border border-primary ${fontDrop ? "visible" : "hidden"}`}>
              {fonts.map((value, index) => {
                return <div className={`${index !== 2 ? "border-b" : "rounded-b-md"} px-2 py-2  hover:bg-gray-200 ${value}`} key={index} onClick={() => { return setFont(value), theme.updateFont(value) }
                }>{value}</div>
              })}
            </div>
          </button>
          ...
          >{weight}
            <div className={`absolute flex flex-col top-full -left-[1px] w-24 rounded-b-md text-gray-800 bg-white  border border-primary ${weightDrop ? "visible" : "hidden"} `}>
              {weights.map((value, index) => {
                return <div className={`${index !== 2 ? "border-b" : "rounded-b-md"} px-1 py-2 hover:bg-gray-200 ${font} ${value}`} key={index} onClick={() => { return setWeight(value), theme.updateWeight(weight) }}>{value}</div>
              })}
            </div>
          </button>
        </div>
        ...
        <div className="flex flex-row gap-4">
          {
            Colors.map((value, index) => {
              return <button onClick={() => (setColor(value), theme.updateBackgroundColor(value))} key={index} className={`size-8 ${value} rounded-full border ${color == value && `ring-offset-0 ring-2 ring-black/30`} `}></button>
            })
          }
        </div>
      </div>
