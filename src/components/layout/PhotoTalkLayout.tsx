...
const PhotoTalkLayout = ({
  ...
  return (
    <div className="bg-surface dark:bg-surface-dark max-w-[520px] min-h-screen m-auto">
      <header className="fixed top-0 left-0 right-0 z-20 m-auto max-w-[520px] flex-between h-12 backdrop-blur-3xl bg-surface-muted dark:bg-surface-muted-dark text-white">
        <button
          onClick={handleBack}
          aria-label="뒤로가기"
          className="p-2 text-label dark:text-label-dark"
        >
          <BackIcon />
        </button>
        <h1
          className={`absolute left-1/2 transform -translate-x-1/2 text-label dark:text-label-dark ${titleStyle}`}
        >
          {title}
        </h1>
      </header>
